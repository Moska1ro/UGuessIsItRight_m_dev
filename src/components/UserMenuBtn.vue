<template>
  ...
  <v-dialog v-model="settingDialog.show" max-width="400px" persistent>
    <v-card>
      <v-card-title>账号设置</v-card-title>
      <v-card-text>
        <v-form ref="settingForm">
          <v-text-field label="UID" density="compact" readonly v-model="settingDialog.uid"></v-text-field>
          <v-text-field label="账号名称" density="compact" :rules="[requiredRule('名称')]"
            v-model="settingDialog.name"></v-text-field>
        </v-form>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" variant="text" @click="settingDialog.show = false">
          取消
        </v-btn>
        <v-btn color="primary" variant="text" @click="confirmSettingDialog" :loading="settingDialog.loading">
          保存
        </v-btn>
      </v-card-actions>
    </v-card>
  ...
  <v-dialog v-model="passwdDialog.show" max-width="400px" persistent>
    <v-card>
      <v-card-title>修改密码</v-card-title>
      <v-card-text>
        <v-form ref="passwdForm">
          <v-text-field label="原密码" density="compact" type="password" :rules="[requiredRule('原密码')]"
            v-model="passwdDialog.oldPassword"></v-text-field>
          <v-text-field label="新密码" density="compact" type="password" :rules="[requiredRule('新密码')]"
            v-model="passwdDialog.newPassword"></v-text-field>
          <v-text-field label="确认密码" density="compact" type="password" :rules="[confirmPasswdRule]"
            v-model="passwdDialog.confirmPassword"></v-text-field>
        </v-form>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
        <v-btn color="primary" variant="text" @click="passwdDialog.show = false">
          取消
        </v-btn>
        <v-btn color="primary" variant="text" @click="confirmPasswdDialog" :loading="passwdDialog.loading">
          保存
        </v-btn>
      </v-card-actions>
    </v-card>
